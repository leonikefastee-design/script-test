...
local d=game:GetService("ReplicatedStorage"):WaitForChild("Datas",30)
_AD=require(d:WaitForChild("Animals",30))
_MD=require(d:WaitForChild("Mutations",30))
_TD=require(d:WaitForChild("Traits",30))
end)
...
do
  if not _G.__SXENetBuilt then
    ...
    local netFolder = ReplicatedStorage:WaitForChild("Packages", 30):WaitForChild("Net", 30)
...
do
    local _netFolder = ReplicatedStorage:WaitForChild("Packages", 30):WaitForChild("Net", 30)
...
do
    ...
    applyCap()
    -- only keep the re-apply loop alive if setfpscap actually exists;
    -- no point burning a task.wait(2) loop every session where it doesn't
    if setfpscap then
        task.spawn(function()
            while true do
                task.wait(2)
...
local function makeOneWay(plat)
    ...
    -- cache outside the hot Stepped path
    local _RS  = game:GetService("RunService")
    local _LP  = game:GetService("Players").LocalPlayer
    local conn
    conn = _RS.Stepped:Connect(function()
        ...
        -- moving up: let the player pass through
        if hrp.AssemblyLinearVelocity.Y > 0.5 then
            plat.CanCollide = false
        -- player is above the platform top: solid
        elseif hrp.Position.Y - hrp.Size.Y / 2 >= plat.Position.Y + plat.Size.Y / 2 - 0.5 then
            plat.CanCollide = true
        else
            plat.CanCollide = false
        end
...
do
    local RS = game:GetService("ReplicatedStorage")
    local netFolder = RS:WaitForChild("Packages", 30):WaitForChild("Net", 30)
    ...
    _G.SXEFireGrapple2 = function(value, exato)
        ...
        --  SXEGrappleSubirEHook. Com ele removido, ninguem mais
    ...
    _G.SXECicloTP = function(esperaMax)
        ...
        if not temAlvo() then
            ...
            warn(("[SXE] ciclo TP: nenhum alvo no cache apos %.1fs -- "
        ...
        --  SXEFireGrapple2 entra no modo de BUSCA: pega a direcao
...
task.defer(LPH_NO_VIRTUALIZE(function()
    -- fast burst: drain the initial queue with minimal yielding (one frame per item)
    -- so startup features all register quickly, then throttle to 0.08s once idle
    local _burst = true
    task.delay(3, function() _burst = false end)
    while true do
        if #_G.__SXELazyQ > 0 then
            ...
            if _burst then
                task.wait() -- yield one frame, not 0.08s, during burst
            else
                task.wait(0.08)
            end
        else
...
-- ============================================================
--  BARRA DO TOPO  --  Heresy Hub | FPS/PING | discord
--
...
HERESY = { NOME="HERESY", GRAD_A=Color3.fromRGB(139,92,246), GRAD_B=Color3.fromRGB(232,84,168) }
...
saveConfig = function()
    ...
    pcall(function()
        ...
        if ok and json then writefile(CONFIG_FILE, json) end
    end)
end
...
local function initToggles()
    ...
    Config.ClickToAP = Config.ClickToAP ~= false  -- on by default for a fresh start, but respect saved config
...
local function apIsOnCooldown(cmd)
    ...
    local l=lastActionUse[cmd]; local cd=ACTION_COOLDOWNS[cmd] or 0; return l and cd>0 and (os.clock()-l)<cd
end
local function apGetRemaining(cmd)
    local realTime = _readRealAdminTimer(cmd)
    if realTime ~= nil then return realTime end
    local l=lastActionUse[cmd]; local cd=ACTION_COOLDOWNS[cmd] or 0; if not l then return 0 end; return math.max(0,cd-(os.clock()-l))
end
local function apStartCooldown(cmd) lastActionUse[cmd]=os.clock() end
...
local function instantReset()
    ...
    if os.clock() - _ultimoReset < 8 then return end  -- raised from 2s: full respawn+TP cycle takes 1-3s, 2s was too short and caused reset spam
...
executeReset = function(isBalloon)
    if isBalloon then
        if os.clock() - lastBalloonResetTime < 20 then return end
        lastBalloonResetTime = os.clock()
    end
    instantReset()
end
...
local _wfCoroutines={}
local function stopWalkFling() _wfActive=false; for _,c in ipairs(_wfConns) do if typeof(c)=="RBXScriptConnection" then c:Disconnect() end end; _wfConns={}; _wfCoroutines={} end
local function startWalkFling()
    ...
    local co=coroutine.create(function()
        ...
        while _wfActive do RunService.Heartbeat:Wait(); if not rr or not rr.Parent then break end
            local v=rr.AssemblyLinearVelocity; rr.AssemblyLinearVelocity=v*10000+Vector3.new(0,10000,0)
            RunService.RenderStepped:Wait(); if rr then rr.AssemblyLinearVelocity=v end
            RunService.Stepped:Wait(); if rr then rr.AssemblyLinearVelocity=v+Vector3.new(0,0.1,0) end
        end
    end); coroutine.resume(co); table.insert(_wfCoroutines,co)
end
...
local function createErrorOrb()
    ...
    errorOrbActive = true
    for _, ghost in ipairs(serverGhosts) do if ghost and ghost.Parent then ghost:Destroy() end end
    serverGhosts = {}
...
local function createServerGhost(position)
    ...
    for _, g in ipairs(serverGhosts) do if g and g.Parent then g:Destroy() end end
    serverGhosts = {}
...
local function clearAllGhosts()
    for _, ghost in ipairs(serverGhosts) do pcall(function() if ghost and ghost.Parent then ghost:Destroy() end end) end
...
local function revertClone()
    local character = player.Character
    if not oldRoot or not oldRoot:IsDescendantOf(Workspace) or not character or not character:FindFirstChild("Humanoid") or character.Humanoid.Health <= 0 then return end
    local tmp = Instance.new("Model"); tmp.Parent = game
    character.Parent = tmp
...
-- Forward declarations: WalkSpeedState and setWalkSpeedEnabled are defined later
-- but invisTurnOff/invisTurnOn (below) reference them early.
local WalkSpeedState
local setWalkSpeedEnabled
...
_G.toggleInvisibleSteal = function()
    if (os.clock() - _invisToggleCooldown) < 0.3 then return end
...
setWalkSpeedEnabled = function(en)
...
local function setInfiniteJump(en) InfJumpState.enabled=en; Config.InfiniteJump=en; Config.TpSettings.InfiniteJump=en; setToggle("Infinite Jump",en); saveConfig()
    ...
    InfJumpState.conn=RunService.Heartbeat:Connect(function() if not UIS:IsKeyDown(Enum.KeyCode.Space) then return end
        local now=os.clock(); if now-InfJumpState.lastJump<0.1 then return end; local c=player.Character; if not c then return end
...
if not Packages or not Datas then
    warn("[SXE AutoSteal] Packages or Datas not found within 30s -- auto steal disabled")
    return
end
local Synchronizer = require(Packages:WaitForChild("Synchronizer", 30))
local AnimalsData = require(Datas:WaitForChild("Animals", 30))
if not Synchronizer or not AnimalsData then
    warn("[SXE AutoSteal] Synchronizer or AnimalsData failed to load -- auto steal disabled")
    return
end
...
RunService.RenderStepped:Connect(function()
    ...
    if status.active then
        local p = math.clamp((os.clock() - (status.start or 0)) / (status.duration or 1.3), 0, 1)
    ...
    task.spawn(function()
        ...
        if modelsFolder and brainrotName then
            ...
            if sourceModel then
                ...
                -- Move first, THEN measure: GetBoundingBox before moving returns
                -- stale world-space coords that are useless after repositioning.
                ...
                local boundCFrame, size = modelClone:GetBoundingBox()
                local modelCenter = boundCFrame.Position  -- accurate post-move center
                local maxAxis = math.max(size.X, size.Y, size.Z)
                
                -- Zoomed in camera offset: position relative to model center and aim at it
                local cameraOffset = modelCenter + Vector3.new(0, size.Y / 2, -(maxAxis * 0.85))
                camera.CFrame = CFrame.lookAt(cameraOffset, modelCenter)
...
task.spawn(function()
    ...
    -- arquivo (2247, 4247, 9196, o _G.SXE_GetPlotAnimalList da linha 271) e
    ...
    local function setupPlotListener(plot, phase) local ch; local retries=0
        ...
        --
        --  Era assim:
        --      ... _G.SXE_GetPlotChannel(plot.Name) ...
...
local function loadModules()
    ...
    local ok = pcall(function()
        ...
        Synchronizer = require(Packages:WaitForChild("Synchronizer", 15))
        AnimalsData = require(Datas:WaitForChild("Animals", 15))
        AnimalsShared = _G._xenAnimShim
        NumberUtils = require(Utils:WaitForChild("NumberUtils", 15))
    end)
...
local function fireGrapple(destino)
    ...
    -- mira propria + Activated (ver SXEFireGrapple2)
...
-- foi removido: o hook e engatado antes pelo SXEGrappleSubirEHook,
...
function runAutoSnipe()
    ...
    local _ok1, lastFpsCap = pcall(getfpscap); if not _ok1 then lastFpsCap = 999 end
    pcall(setfpscap, 200)
        ...
        local signPart = getClosestBaseSign(targetPart)
            ...
            if Config.TpSettings.FlyTP then
                ...
                if isCloseBase then
                    ...
                    local duration = 2.5
                    local start = os.clock()
                    while os.clock() - start < duration and hrp.Parent do
            ...
            local holdEnd = os.clock() + math.max(0.18, Config.TpSettings.DelayVal or 0.4)
...
function tpToBrainrot()
    ...
    local _ok2, lastFpsCap = pcall(getfpscap); if not _ok2 then lastFpsCap = 60 end
    pcall(setfpscap, 200)
    local ok, err = pcall(function()
        local carpetName = Config.TpSettings.Tool
        local carpet = LocalPlayer.Backpack:FindFirstChild(carpetName) or char:FindFirstChild(carpetName)
...
local function buildRemoteSell()
    ...
    pcall(function()
        ...
        if Pkgs then Synchronizer_rs=require(Pkgs:WaitForChild("Synchronizer", 15)) end
        if Datas then AnimalsData_rs=require(Datas:WaitForChild("Animals", 15)) end
        if Shared then AnimalsShared_rs=_G._xenAnimShim end
        if Utils then NumberUtils_rs=require(Utils:WaitForChild("NumberUtils", 15)) end
    end)
...
-- mostrava nada. "Heresy Hub" tambem vira a chave de posicao/tamanho
-- salvos (Config.positions[t] / Config.sizes[t]) -- a antiga era ""
-- e nao batia com o applySavedPosition("SXE HUB PRIVAT") logo abaixo,
-- entao a janela nunca restaurava a posicao mesmo.
main,mainBody=makeMainPanel("Heresy Hub",UDim2.new(0,375,0,480),UDim2.new(0.5,-187,0.5,-255))
...
-- "Heresy Hub" aqui tambem: e a mesma chave usada em makeMainPanel.
-- Antes era "SXE HUB PRIVAT" enquanto o painel salvava sob "" --
-- chaves diferentes, entao a posicao nunca voltava.
for _,pair in ipairs({{"Heresy Hub",main},{"SXE HUB PRIVAT\nInvisible Steal",panels["Invisible Steal Panel"]},
...
function loadTab(tabName)
    ...
    elseif tabName=="UI" then
        -- HERESY EXTRAS: guardado porque loadTab pode rodar na inicializacao,
...
-- FPS/PING COUNTER
-- Uses Heartbeat + os.clock() with a 0.5s bucket (twice as responsive as the
-- old 1s RenderStepped/os.clock() approach, and avoids the deprecated os.clock() API).
do
    local _bucket, _last = 0, os.clock()
    _G.currentFPS = 60
    RunService.Heartbeat:Connect(function()
        _bucket = _bucket + 1
        local now = os.clock()
        if now - _last >= 0.5 then
            local fps = math.floor(_bucket / (now - _last) + 0.5)
            _G.currentFPS = fps
            _bucket = 0
            _last   = now
            local ping = 0
            ...
            if fpsText and fpsText.Parent then
                pcall(function() fpsText.Text = "FPS: "..fps.."\nPING: "..ping.."ms" end)
            end
...
do
    ...
    local function fireGrapple(destino)
        ...
        -- Mira propria + Activated. O SXEFireGrapple2 escreve
        ...
        if not (_G.SXEFireGrapple2 and _G.SXEFireGrapple2(destino)) then
            warn("[SXE] fireGrapple: sem ponto de chao valido entre 10 e 50 studs na direcao do alvo")
        end
    end
...
--
--  O hook agora e engatado pelo SXEGrappleSubirEHook, antes do
...
local function carpetEngage()
    ...
    local function buildStealCallbacks(prompt)
        -- Only skip if already cached WITH at least one callback. An empty result
        -- means the game's scripts hadn't connected yet; allow a retry so we don't
        -- permanently blacklist a prompt that just loaded late.
        local existing = InternalStealCache[prompt]
        if existing and (#existing.holdCallbacks > 0 or #existing.triggerCallbacks > 0 or #existing.holdEndCallbacks > 0) then return end
        ...
        -- Preserve existing ready state if we're replacing a previously-empty entry
        if existing and existing.ready == false then data.ready = false end
    ...
    local function executeStealAsync(prompt)
        ...
        if _stealHoldActive and (os.clock() - _stealHoldStart) < (STEAL_HOLD_DURATION + 1) then
            return false
        end
        data.ready = false
        _stealHoldStart = os.clock()
        ...
        task.spawn(function()
            ...
            local remain = _holdDur - (os.clock() - _stealHoldStart)
    ...
    local function doVelocityTP()
        ...
        --
        -- Quem engata o hook agora e o SXEGrappleSubirEHook, na ordem
